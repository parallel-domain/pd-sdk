...
def pd_cli():
    parser = argparse.ArgumentParser(description="CLI interface of pd-sdk. Lets you visualize data")
    subparsers = parser.add_subparsers(dest="cmd", help="available commands: [visualize]")

    s_parser = subparsers.add_parser("visualize", help="visualize a stored dataset")
    _add_visualize_commands(parser=s_parser)

    args, unknown = parser.parse_known_args()
    if args.cmd == "visualize":
        _visualize_dataset_call(args=args, unknown=unknown)
    else:
        raise ValueError(f"Unknown command {args.cmd}")


def _visualize_dataset_call(args: Any, unknown: List[str]):
    kwargs = dict()
    for arg in unknown:
        if arg.startswith(("-", "--")):
            k, v = arg.split("=")
    ...
    dataset = decode_dataset(dataset_path=args.input_path, dataset_format=args.dataset_format, **kwargs)
    dataset = data_transform.apply(dataset)
    show_dataset(dataset)


def _add_visualize_commands(parser: argparse.ArgumentParser):
    parser.add_argument("input_path", type=str, help="Path to the dataset location (s3 or local)")
    parser.add_argument(
        "-f",
        "--dataset_format",
        type=str,
