...
class SemanticSegmentation2DEncoderStep(EncoderStep):
    def __init__(
        self,
        fs_copy: bool,
        workers: int = 1,
    ...
    def apply(self, scene: Scene, input_stage: Iterable[Dict[str, Any]]) -> Iterable[Dict[str, Any]]:
        stage = pypeln.thread.map(
            f=self.encode_semantic_segmentation_2d, stage=input_stage, workers=self.workers, maxsize=self.in_queue_size
        )
