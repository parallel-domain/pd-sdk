...
class SurfaceNormals2DEncoderStep(EncoderStepHelper, EncoderStep):
    def __init__(
        self,
        fs_copy: bool,
        workers: int = 1,
    ...
    def apply_stages(self) -> bool:
        return self.output_annotation_types is None or AnnotationTypes.SurfaceNormals2D in self.output_annotation_types

    def encode_surface_normals_2d(self, input_dict: Dict[str, Any]) -> Dict[str, Any]:
        sensor_frame = self._get_camera_frame_from_input_dict(input_dict=input_dict)
        if sensor_frame is not None and AnnotationTypes.SurfaceNormals2D in sensor_frame.available_annotation_types:
            ...
            sn_file_path = sensor_frame.get_file_path(data_type=FilePathedDataType.SurfaceNormals2D)
            ...
            input_dict["annotations"][str(ANNOTATION_TYPE_MAP_INV[AnnotationTypes.SurfaceNormals2D])] = save_path
        return input_dict
    ...
    def apply(self, input_stage: Iterable[Dict[str, Any]]) -> Iterable[Dict[str, Any]]:
        ...
        if self.apply_stages:
            stage = pypeln.thread.map(
                f=self.encode_surface_normals_2d, stage=input_stage, workers=self.workers, maxsize=self.in_queue_size
            )
