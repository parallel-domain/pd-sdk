...
class BoundingBoxes3DEncoderStep(EncoderStepHelper, EncoderStep):
    def __init__(
        self,
        workers: int = 1,
        in_queue_size: int = 4,
    ...
    def apply_stages(self) -> bool:
        return self.output_annotation_types is None or AnnotationTypes.BoundingBoxes3D in self.output_annotation_types
    ...
    def save_boxes_3d_to_file(
        ...
        return str(fsio.write_json_message(obj=boxes3d_dto, path=output_path, append_sha1=True))

    def apply(self, input_stage: Iterable[Dict[str, Any]]) -> Iterable[Dict[str, Any]]:
        stage = input_stage
        if self.apply_stages:
            # seems to be the slowest step among the annotation types.
            # Hence process instead of thread to make sure its not blocking the others
