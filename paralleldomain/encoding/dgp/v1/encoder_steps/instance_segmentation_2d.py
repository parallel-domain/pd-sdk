...
class InstanceSegmentation2DEncoderStep(EncoderStepHelper, EncoderStep):
    def __init__(
        self,
        fs_copy: bool,
        workers: int = 1,
    ...
    def apply_stages(self) -> bool:
        return (
            ...
            or AnnotationTypes.InstanceSegmentation2D in self.output_annotation_types
        )
    ...
    def apply(self, input_stage: Iterable[Dict[str, Any]]) -> Iterable[Dict[str, Any]]:
        ...
        if self.apply_stages:
            stage = pypeln.thread.map(
                f=self.encode_instance_segmentation_2d,
                stage=input_stage,
