...
from paralleldomain.model.annotation import AnnotationType, AnnotationTypes, BoundingBox2D, Point2D, Points2D
...
class Points2DEncoderStep(EncoderStepHelper, EncoderStep):
    ...
    def encode_points_2d(self, input_dict: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if sensor_frame is not None and AnnotationTypes.Points2D in sensor_frame.available_annotation_types:
            ...
            output_path = self.save_points_2d(points=points2d, sensor_frame=sensor_frame, input_dict=input_dict)
    ...
    def save_points_2d(
        self, points: Points2D, input_dict: Dict[str, Any], sensor_frame: SensorFrame[datetime]
    ) -> AnyPath:
        keypoint2d_dto = [self._encode_key_point_2d(p) for p in points.points]
