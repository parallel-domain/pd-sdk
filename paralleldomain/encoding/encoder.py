...
from paralleldomain.model.type_aliases import SensorFrameSetName, SensorName
...
class SceneEncoder:
    def __init__(
        ...
        self._set_name: SensorFrameSetName = set_name
        self._output_path: AnyPath = output_path
        self._sensor_frame_set: SensorFrameSet = dataset.get_sensor_frame_set(set_name=set_name)
        self._task_pool: ThreadPool = ThreadPool(processes=max(int(os.cpu_count() * 0.75), 1))

        self._camera_names: Union[List[str], None] = (
            self._sensor_frame_set.camera_names if camera_names is None else camera_names
        )
        self._lidar_names: Union[List[str], None] = (
            self._sensor_frame_set.lidar_names if lidar_names is None else lidar_names
        )
        self._annotation_types: Union[List[AnnotationType], None] = (
            self._sensor_frame_set.available_annotation_types if annotation_types is None else annotation_types
        )
        ...
        with ThreadPoolExecutor(max_workers=10) as camera_frame_executor:
            camera_frame_executor.map(
                lambda fid: self._encode_camera_frame(
                    self._sensor_frame_set.get_frame(frame_id=fid).get_sensor(sensor_name=camera_name)
                ),
                self._sensor_frame_set.frame_ids,
            ),
            ...
            for frame_id, lidar_frame_encoder_result in zip(
                ...
                lidar_frame_executor.map(
                    lambda fid: self._encode_lidar_frame(
                        self._sensor_frame_set.get_frame(frame_id=fid).get_sensor(sensor_name=lidar_name)
                    ),
                    self._sensor_frame_set.frame_ids,
                ),
            ):
    ...
    def _run_encoding(self) -> Any:
        ...
        logger.info(f"Successfully encoded {self._set_name}")
...
class DatasetEncoder:
    def __init__(
        ...
        if set_names is not None:
            for sn in set_names:
                if sn not in self._dataset.sensor_frame_set_names:
            ...
            self._set_names = self._dataset.sensor_frame_set_names[set_slice]

    def _call_scene_encoder(self, set_name: str) -> Any:
        encoder = self._scene_encoder(
            dataset=self._dataset,
            set_name=set_name,
            output_path=self._output_path / set_name,
    ...
    def encode_dataset(self):
        with ThreadPoolExecutor(max_workers=self._n_parallel) as scene_executor:
            for set_name, scene_encoder_result in zip(
                self._set_names,
                scene_executor.map(
                ...
                logger.info(f"{set_name}: {scene_encoder_result}")
