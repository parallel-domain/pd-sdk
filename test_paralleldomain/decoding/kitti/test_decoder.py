...
def test_can_load_scene(kitti_dataset_train_scenes: List, kitti_train_dataset: Dataset):
    for scene_name in kitti_dataset_train_scenes:
        scene = kitti_train_dataset.get_unordered_scene(scene_name=scene_name)
...
def test_decode_camera_image(kitti_dataset_train_scenes: List, kitti_train_dataset: Dataset):
    for i in range(5):
        scene_name = kitti_dataset_train_scenes[i]
        scene = kitti_train_dataset.get_unordered_scene(scene_name=scene_name)
        camera_frame = next(scene.camera_frames)
        ...
        assert rgb.shape[0] > 365
        assert rgb.shape[1] > 1230
...
def test_decode_optical_flow(kitti_dataset_train_scenes: List, kitti_train_dataset: Dataset):
    for i in range(5):
        scene_name = kitti_dataset_train_scenes[i]
        scene = kitti_train_dataset.get_unordered_scene(scene_name=scene_name)
        camera_frame = scene.get_frame(frame_id=scene.frame_ids[0]).get_sensor(sensor_name="default")
        assert camera_frame is not None
        assert isinstance(camera_frame, CameraSensorFrame)
        ...
        assert vectors.shape[0] > 365
        assert vectors.shape[1] > 1230
        assert vectors.shape[2] == 2
        assert np.all(np.logical_and(flow.vectors.max() < 1024, flow.vectors.min() >= 0))


def test_decode_camera_image_next(kitti_dataset_train_scenes: List, kitti_train_dataset: Dataset):
    scene_name = kitti_dataset_train_scenes[0]
    scene = kitti_train_dataset.get_unordered_scene(scene_name=scene_name)
    dataset_path = scene.metadata["dataset_path"]
    image_folder = scene.metadata["image_folder"]
    for i in range(5):
        scene_name = kitti_dataset_train_scenes[i]
        scene = kitti_train_dataset.get_unordered_scene(scene_name=scene_name)
        camera_frame = next(scene.camera_frames)
        ...
        assert next_image.shape[0] > 365
        assert next_image.shape[1] > 1230
