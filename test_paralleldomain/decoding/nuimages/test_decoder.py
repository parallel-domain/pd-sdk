...
from test_paralleldomain.decoding.constants import NUIMAGES_DATASET_PATH_ENV


@pytest.fixture
def nuimages_dataset_path() -> str:
    if NUIMAGES_DATASET_PATH_ENV in os.environ:
        return os.environ[NUIMAGES_DATASET_PATH_ENV]
    else:
        pytest.skip()


@pytest.fixture
def nuimages_val_dataset(nuimages_dataset_path: str) -> Dataset:
    decoder = NuImagesDatasetDecoder(dataset_path=nuimages_dataset_path, split_name="v1.0-val")
    dataset = decoder.get_dataset()
    return dataset


@pytest.fixture
def nuimages_train_dataset(nuimages_dataset_path: str) -> Dataset:
    decoder = NuImagesDatasetDecoder(dataset_path=nuimages_dataset_path, split_name="v1.0-train")
    dataset = decoder.get_dataset()
    return dataset


@pytest.fixture
def nuimages_mini_dataset(nuimages_dataset_path: str) -> Dataset:
    decoder = NuImagesDatasetDecoder(dataset_path=nuimages_dataset_path, split_name="v1.0-mini")
    dataset = decoder.get_dataset()
    return dataset


class TestDataset:
    def test_decode_val_scene_names(self, nuimages_val_dataset: Dataset):
        assert len(nuimages_val_dataset.scene_names) == 82

    def test_decode_train_scene_names(self, nuimages_train_dataset: Dataset):
        assert len(nuimages_train_dataset.scene_names) == 350


@pytest.fixture
def two_cam_scene(nuimages_val_dataset: Dataset) -> Scene:
    return nuimages_val_dataset.get_scene(scene_name="c6b4b836e4c543378e340a8a28760ebd")
...
class TestFrame:
    def test_decode_frame_ego_pose(self, nuimages_mini_dataset: Dataset):
        scene_names = nuimages_mini_dataset.scene_names
        scene = nuimages_mini_dataset.get_scene(scene_name=scene_names[0])
...
class TestCameraSensorFrame:
    ...
    def test_data_loader(self, nuimages_val_dataset: Dataset):
        for scene_name in nuimages_val_dataset.scene_names[:5]:
            scene = nuimages_val_dataset.get_scene(scene_name=scene_name)
    ...
    @pytest.mark.skip("For debugging")
    def test_data_loader_with_data_loading(self, nuimages_val_dataset: Dataset):
        for scene_name in nuimages_val_dataset.scene_names[:2]:
            scene = nuimages_val_dataset.get_scene(scene_name=scene_name)
            for frame in scene.frames[:5]:
                for camera in frame.camera_frames:
                    if AnnotationTypes.SemanticSegmentation2D in camera.available_annotation_types:
                        ...
                        assert (
                            camera.get_annotations(annotation_type=AnnotationTypes.SemanticSegmentation2D).class_ids
                            is not None
                        )
