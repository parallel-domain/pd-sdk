...
from paralleldomain.data_lab import DEFAULT_DATA_LAB_VERSION, Location, SensorConfig, SensorRig, preview_scenario
...
class NearbyVehiclesOnDriveways(ScenarioCreator):
    def create_scenario(
        ...
        # We will use a while loop to search for a spawn location that has sufficient surrounding driveways

        # Create a trigger variable that allows us to exit the while loop when a spawn point has been found
        ...
        while not valid_spawn_found:
            ...
            if (
                np.linalg.norm(driveway_reference_line[-1] - driveway_reference_line[0])
                < 1.15 * minimum_driveway_length
            ):
                continue
            ...
            if spawn_point_segment is None:
                logger.info(
                    f"Failed to find valid spawn point at seed {scenario.random_seed} - incrementing and retrying"
                )
                ...
                # Set our trigger variable to True so that we can exit the search loop
                valid_spawn_found = True

            # If we exceed 1000 attempts to search for a spawn location, we exit and return an error
            if retries >= 1000:
                raise PdError(
                    "Failed to find valid spawn location within max_retries set.  "
                    "Adjust max_retries or spawn conditions"
                )
