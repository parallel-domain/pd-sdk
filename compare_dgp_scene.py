...
from compare_dgp_scene_utils import (
    diff_images,
    write_image,
    calculate_iou,
    report_errors,
    compare_attribute_by_key,
    difference_between_vertices,
    get_instance_dicts,
    map_test_to_target,
)
    ...
    def pytest_generate_tests(self, metafunc):
        ...
        if "frame_pair" in metafunc.fixturenames:
            ...
            if num_frames_test_scene != num_frames_target_scene:
                raise Exception(
                    f"Mismatch in # frames between test scene ({num_frames_test_scene}) "
        ...
        if "camera_name" in metafunc.fixturenames:
            ...
            if test_camera_names != target_camera_names:
                raise Exception(
                    f"Mismatch in test camera names ({test_camera_names}) "
...
def test_ontology(scene_pair):
    ...
    if set(test_scene.available_annotation_types) != set(target_scene.available_annotation_types):
        print(
            f"Available annotation types mismatch:\n {test_scene.available_annotation_types}\n\n {target_scene.available_annotation_types}\n"
        )
        # Don't fail on these

    common_annotation_types = set(test_scene.available_annotation_types).intersection(
        target_scene.available_annotation_types
    )
...
def test_camera_bbox2d(camera_frame_pair):
    ...
    if len(test_bbox2d_boxes) != len(target_bbox2d_boxes):
        general_errors.append(
            "The length of bounding boxes is not equal. There are {} target boxes while the test has {}".format(
                len(target_bbox2d_boxes), len(test_bbox2d_boxes)
            )
        )
    ...
    for target_box in target_bbox2d_boxes:
        ...
        if best_match == None or target_box.class_id != best_match.class_id or not less_then_minimum_iou:
            no_test_box_for_target.append(
                "Could not find a match for the target bounding box {}, Areas is {}.".format(
                    target_box, target_box.area
                )
            )
            continue
        ...
        if test_instance_id_target_match_pair.get(best_match_key, -1) != -1:
            ...
            test_matches_two_targets.append(
                "The following test box {} is the cloest match for the two target boxes {} and {}".format(
                    best_match, target_box, other_target_box
                )
            )
            continue
        ...
        if len(attribute_errors) != 0:
            test_target_attribute_mismatch.append(
                "The test bounding box {} and target bounding box {} had the following attribute errors:\n\t{}".format(
                    best_match, target_box, "\n\t".join(attribute_errors)
                )
            )
        ...
        if (
            ...
            test_target_match_different_sizes.append(
                "The test box {} and target box {} have different sizes of {} and {}  with a abs percent diff {}. Class id {}".format(
                    test_box.instance_id,
                    target_box.instance_id,
                    test_box.area,
                    target_box.area,
                    percent_diff,
                    test_box.class_id,
    ...
    for test_box in test_bbox2d_boxes:
        if test_box.instance_id not in matched_test_box_instance_ids:
            no_target_box_for_test.append(
                "Could not find a match for the test bounding box {}. Area is {}.".format(test_box, test_box.area)
            )
    ...
    if len(all_errors) != 0:
        ...
        print(
            ...
            "Out of {} matched boxes. {:3.2f}% have attribute mismatches\n".format(
                len(general_errors),
                number_test_boxes,
                percentage_test_boxes_matched,
                number_target_boxes,
                percentage_target_boxes_matched,
                len(test_matches_two_targets),
                boxes_matched,
                percentage_of_matched_boxes_with_attribute_mismatch,
            )
        )
...
def test_camera_bbox3d(camera_frame_pair):
    ...
    target_bbox3d_boxes = [
        x for x in target_bbox3d_boxes if x.volume > MIN_3D_BBOX_VOLUME and x.attributes.get("truncation", -1) != 1
    ]
    ...
    if len(test_bbox3d_boxes) != len(number_non_zero_boxes):
        general_errors.append(
            "The length of bounding boxes is not equal. There are {} target boxes while the test has {}".format(
                len(number_non_zero_boxes), len(test_bbox3d_boxes)
            )
        )
    ...
    for target_box in target_bbox3d_boxes:
        ...
        if best_match == None:
            no_test_box_for_target.append(
                "Could not find a match for the target bounding box {}, Volume {}.".format(
                    target_box, target_box.volume
                )
            )
            continue
        ...
        if test_instance_id_target_match_pair.get(best_match_key, -1) != -1:
            other_target_box = test_instance_id_target_match_pair.get(best_match_key)
            test_matches_two_targets.append(
                "The following test box {} is the closest match for the two target boxes {} and {}".format(
                    best_match, target_box, other_target_box
                )
            )
            continue
    ...
    for test_box in test_bbox3d_boxes:
        if test_box.instance_id not in matched_test_box_instance_ids:
            no_target_box_for_test.append(
                "Could not find a match for the test bounding box {}. Volume is {}.".format(test_box, test_box.volume)
            )
    ...
    if len(all_errors) != 0:
        ...
        # Stats
        print(
            "There was {} test and {} target boxes before filtering".format(
                pre_filter_no_of_test_boxes, pre_filter_no_of_target_boxes
            )
        )
        print(
            "There are {} general errors.\n"
...
def test_camera_semseg2d(camera_frame_pair, output_dir):
    ...
    class_diff = np.setdiff1d(
        np.unique(test_semseg2d.class_ids.flatten()), np.unique(target_semseg2d.class_ids.flatten())
    )
...
def test_camera_instanceseg2d(camera_frame_pair, output_dir):
    ...
    test_to_target_map, unmatched_test_instances, unmatched_target_instances = map_test_to_target(
        test_instances,
